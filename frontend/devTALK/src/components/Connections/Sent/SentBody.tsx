...
import { getSentRequests } from "../../../api/ConnectionAPI.ts";
import { StatusCodes } from "http-status-codes";
import { errorMessageService } from "../../../contexts/ErrorMessageService.ts";
import SentCard from "./SentCard.tsx";

function SentBody({ currentlySelected }: { currentlySelected: string }) {
	...
	const fetchConnectionRequests = async (pageNumber: number) => {
		setLoading(true);
		const response = await getSentRequests(pageNumber);
		setLoading(false);
	...
	return (
		<div>
			<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4">
				{currentlySelected === "sent" &&
					requests.map((request, index) => (
						<SentCard key={index} request={request} />
					))}
			</div>
			{currentlySelected === "sent" && (
				<div ref={observer} className="text-center  items-center my-4">
					...
					{!hasMore && !loading && (
						<p className="text-gray-500 mt-4">
							No more sent requests available
						</p>
					)}
				</div>
			)}
